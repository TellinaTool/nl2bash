...
from src.bashlint import bash
from src.encoder_decoder import slot_filling
from src.data_processor import data_utils
from src.eval import tree_dist
from src.nlp_tools import constants, format_args, tokenizer
...
def decode(model_outputs, FLAGS, vocabs, sc_fillers=None,
    ...
    """
    rev_cm_vocab = vocabs.rev_cm_vocab
    ...
    for batch_id in xrange(batch_size):
        def as_str(output, r_cm_vocab):
            if output < FLAGS.cm_vocab_size:
                token = r_cm_vocab[output]
            else:
                if FLAGS.use_copy and FLAGS.copy_fun == 'copynet':
                    source_id = output - FLAGS.cm_vocab_size
        ...
        for beam_id in xrange(len(top_k_predictions)):
            ...
            for token_id in xrange(len(outputs)):
                ...
                pred_token = as_str(output, rev_cm_vocab)
                ...
                if pred_token in bash.argument_types:
                    if token_id > 0 and format_args.is_min_flag(
                        rev_cm_vocab[outputs[token_id-1]]):
            ...
            if FLAGS.channel == 'char':
                ...
                for char in output_tokens:
                    if char == src.nlp_tools.constants._SPACE:
                        target += ' '
                    else:
                        target += char
            else:
...
def decode_set(sess, model, dataset, top_k, FLAGS, verbose=False):
    ...
    vocabs = data_utils.load_vocabulary(FLAGS)
    rev_nl_vocab = vocabs.rev_nl_vocab
    ...
    for example_id in xrange(len(grouped_dataset)):
        ...
        nl_tokens = [rev_nl_vocab[i] for i in data_group[0].nl_ids]
        if FLAGS.channel == 'char':
            sc_temp = ''.join(nl_tokens)
...
def query_to_encoder_features(sentence, vocabs, FLAGS):
    ...
    """
    if FLAGS.channel == 'char':
        tokens = data_utils.nl_to_chars(sentence)
        ...
        init_vocab = data_utils.TOKEN_INIT_VOCAB
    nl_ids = data_utils.tokens_to_ids(tokens, vocabs.nl_vocab)
    encoder_features = [[nl_ids]]
    if FLAGS.use_copy and FLAGS.copy_fun == 'copynet':
        copy_sc_ids = []
        for i, t in enumerate(tokens):
            if not t in init_vocab and t in vocabs.cm_vocab:
                copy_sc_ids.append(vocabs.cm_vocab[t])
            else:
                copy_sc_ids.append(len(vocabs.cm_vocab) + i)
        encoder_features.append([copy_sc_ids])
...
# --- Visualization --- #
def visualize_attn_alignments(M, source, target, rev_nl_vocab,
                              rev_cm_vocab, output_path):
