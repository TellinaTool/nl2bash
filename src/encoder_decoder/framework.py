...
from src.encoder_decoder import common
from src.data_processor import data_utils
from src.encoder_decoder.seq2seq import rnn_decoder


DEBUG = False


class EncoderDecoderModel(common.NNModel):
    ...
    def __init__(self, hyperparams, buckets=None):
        ...
        self.define_decoder(decoder_dim, decoder_embedding_dim,
                            self.cm_token_use_attention,
                            self.cm_token_attn_fun,
                            self.tg_input_keep,
                            self.tg_output_keep)
    ...
    def encode_decode(self, encoder_channel_inputs, encoder_attn_masks,
        ...
        if not self.forward_only:
            ...
            attention_reg = self.attention_regularization(attn_alignments) \
                if self.cm_token_use_attention else 0
            ...
            if self.tg_char:
                ...
                if bs_decoding:
                    char_decoder_inputs = common.wrap_inputs(
                        self.decoder.beam_decoder, char_decoder_inputs)
                    char_targets = common.wrap_inputs(
                        self.decoder.beam_decoder, char_targets)
                    char_target_weights = common.wrap_inputs(
    ...
    def get_batch(self, data, bucket_id=-1, use_all=False):
        ...
        for i in data_ids:
            ...
            encoder_inputs.append(example.nl_ids)
            decoder_inputs.append(example.cm_ids)
            if self.copynet:
                encoder_copy_inputs.append(example.copy_sc_ids)
                copy_targets.append(example.copy_tg_ids)
