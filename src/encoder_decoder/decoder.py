...
class Decoder(common.NNModel):
    ...
    def token_features(self):
        return np.load(self.cm_token_features_path)
...
class CopyCellWrapper(tf.compat.v1.nn.rnn_cell.RNNCell):
    def __init__(self, cell, output_project, num_layers,
                 encoder_copy_inputs, cm_vocab_size):
        self.cell = cell
        self.output_project = output_project
        self.num_layers = num_layers
        self.cm_vocab_size = cm_vocab_size
    ...
    def __call__(self, input_embedding, state, scope=None):
        ...
        gen_prob = tf.slice(prob, [0, 0], [-1, self.cm_vocab_size])
        copy_prob = tf.slice(prob, [0, self.cm_vocab_size], [-1, -1])
        copy_vocab_prob = tf.squeeze(tf.matmul(tf.expand_dims(copy_prob, 1),
            tf.one_hot(self.encoder_copy_inputs, depth=self.cm_vocab_size+copy_prob.get_shape()[1])), 1)
...
class AttentionCellWrapper(tf.compat.v1.nn.rnn_cell.RNNCell):
    def __init__(self, cell, attention_states, encoder_attn_masks,
            ...
            num_heads, dim, num_layers, use_copy, cm_vocab_size=-1):
        """
        ...
        self.vocab_indices = tf.linalg.tensor_diag(tf.ones(cm_vocab_size))
    ...
    def __call__(self, input_embedding, state, scope=None):
        ...
        with tf.compat.v1.variable_scope("AttnOutputProjection"):
            output = common.linear([cell_output, attns[0]], self.dim, True)
