    ...
    for bucket_id in xrange(len(model.buckets)):
        for i in xrange(len(dataset.examples[bucket_id])):
            ...
            if dp.alignments is not None:
                ...
                encoder_channel_inputs = [[dp.nl_ids]]
                decoder_channel_inputs = [[dp.cm_ids]]
                # print(bucket_id)
                # print(encoder_channel_inputs)
                # print(decoder_channel_inputs)
                if FLAGS.use_copy:
                    encoder_channel_inputs.append([dp.copy_sc_ids])
                    decoder_channel_inputs.append([dp.copy_tg_ids])
