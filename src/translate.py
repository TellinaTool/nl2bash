...
def define_model(session, forward_only, buckets=None):
    """
    Define tensor graphs.s
    """
    if FLAGS.decoder_topology in ['basic_tree']:
        return common.define_model(
            ...
            if t % FLAGS.epochs_per_checkpoint == 0:
                ...
                if loss < 300:
                    ...
                    if t > 1:
                        ...
                        raise common.InfPerplexityError
...
def main(_):
    ...
    if FLAGS.process_data:
        ...
        if FLAGS.eval:
            ...
            FLAGS.max_cm_seq_len = train_set.max_cm_seq_len if not train_set.buckets else \
                train_set.buckets[-1][1]
            FLAGS.nl_vocab_size = len(vocab.nl_vocab)
            FLAGS.cm_vocab_size = len(vocab.cm_vocab)
            FLAGS.max_nl_token_size = vocab.max_nl_token_size
            FLAGS.max_cm_token_size = vocab.max_cm_token_size
