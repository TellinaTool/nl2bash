...
def score_tool_args(word, tool_args):
    return (1-word_freqs.get(word, 0)) * tool_args.score(word)

def score_tool(word, tool):
    return score_tool_name(word, tool.name) + score_tool_args(word, tool.flags)
...
def run(q):
    ...
    for t in sorted(scores.keys(), key=scores.get, reverse=True):
        ...
        score_func = lambda arg: sum(score_tool_args(w, arg) for w in words)
        print("        {:40} [score={:1.2f}: {}]".format(
            t.name + t.flags.propose_incantation(score_func(t.flags), score_func),
