...
\title{Shell Command Synthesis from Natural Language\cut{\titlenote{(Produces the permission block, and
...
\maketitle
\begin{abstract}
In this work, we propose a framework that allows users to express their intended operations on the UNIX file system in English, and automatically translates them to shell commands. We adopted a data-driven, machine learning based approach following recent work in programming by natural language (PBNL). We train the synthesizer with shell command/English description pairs gathered from the web by crowd workers who are familiar with shell scripting. The system is able to synthesize shell commands that starts with the token {\tt find}, permitting redirection to other command with pipes. This is currently work in progress.
\end{abstract}
...
% \keywords{bash program synthesis, programming by natural language (PBNL), machine learning, crowdsourcing}
