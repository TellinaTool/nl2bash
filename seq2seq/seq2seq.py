...
def attention_beam_decoder(decoder_inputs, initial_state, attention_states, cell,
    ...
    In this context "attention" means that, during decoding, the RNN can look up
    information in the additional tensor attention_states, and it does this by
    focusing on a few entries from the tensor. This function extends attention_decoder
    to use beam search to generate top-K ranked results.
...
def embedding_attention_seq2seq(encoder_inputs, decoder_inputs, rnn_cell,
    ...
    Args:
      ...
      initial_state_attention: If False (default), initial attentions are zero.
        If True, initialize the attentions from the initial state and attention
        states.
      beam_decoder: beam_search decoder.
